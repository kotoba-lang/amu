# ADR 0347 — The JVM route leaves in dependencies-first order, and the trust plane is the first family off it
...
- Related: root ADR-2609111500 (the `.cljk` rename, which left the JVM route unable to
...
falls through to `clojure -M:run` and `kotoba.compiler.cli`. Since the `.cljk` rename
that fall-through cannot load the compiler (`Could not locate kotoba/compiler/cli.clj`),
so a command on it has no working route anywhere — measured 2026-09-11 morning:
