# ADR 0249: Version GitHub merge governance
...
API and requires an exact match after normalization. The required checks are
bound to the GitHub Actions application, strict status checks and administrator
enforcement are enabled, resolved conversations are required, and force pushes
...
Protection was exercised with PR #591: an empty commit carrying `[skip ci]`
