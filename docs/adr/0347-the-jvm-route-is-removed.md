...
| `run`, `measure-runtime` | executing artifacts from the CLI, and the loader identity a receipt's `:runtime` refers to | **ported 2026-09-12** (ADR 0348 iteration 8): kototama-native `005c9b74` has the executor on both hosts; amu declares it under the `:native-run` alias and `bin/amu` puts that closure on the classpath for these two commands only. `test-nbb-run` (25 checks) builds the loader, runs a signed artifact, verifies the receipt, and traps `forever` |
