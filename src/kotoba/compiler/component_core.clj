...
(defn- structural-union-match-adapter
  ...
  (let [used (set (:params function))
        ...
        {:name (:name function)
         ...
         ;; The discriminant's source-level type is deliberately i64 so the
         ;; shared emitter can widen and range-check it; :core-param-types
         ;; below seals its actual ABI type as i32.
    ...
    {:function synthetic-function
     ...
     ;; A joined option/result payload is meaningful only in its active case.
     ;; Validate bool after tag dispatch so an inactive slot does not acquire
     ;; semantics or trap.
     :unchecked-bool-params (if (= :i32 joined-core-type) #{1} #{})}))

(defn- structural-union-match-module
  "Plans a multi-export host-free scalar module containing one or more
  exhaustive structural union matches. Non-match helpers/exports may use the
  same four Canonical scalar types and are emitted in the same core module."
  [kir]
