...
(defn- dispatch! [& args]
  (case (first args)
    ...
    (let [input (second args)
          ...
          ;; Free-text the caller puts on the panel beside the build digest --
          ;; a version, a branch, a date. Deliberately not derived here: this
          ;; packager is byte-compared against a second packaging of the same
          ;; input, so anything it invents from the clock would break that. If
          ;; a caller passes a timestamp, keeping it reproducible is theirs.
          k16-note (or (option args "--k16-note") "")
