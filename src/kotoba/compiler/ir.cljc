...
(defn- invoke-function [function values functions fuel heap call-stack cap-call]
  ...
  (let [param-types (or (:param-types function)
    ...
    (doseq [[parameter runtime-value type] (map vector (:params function) values param-types)]
      (validate-runtime-value! runtime-value type
                               {:function (:name function) :parameter parameter})))
...
(defn eval-expr [form env functions fuel heap call-stack cap-call]
  (cond
    ...
    (string? form) (value/bounded-string! form value/string-literal-byte-limit)
    ...
    (let [[op & args] form]
      (cond
        ...
        (#?(:clj long :cljs i64/->bigint)
         (value/utf8-byte-count!
          (eval-expr (first args) env functions fuel heap call-stack cap-call)))

        (= op 'string=?)
        (if (apply = (mapv #(eval-expr % env functions fuel heap call-stack cap-call) args))
          #?(:clj 1 :cljs i64/one)
          #?(:clj 0 :cljs i64/zero))

        (= op 'string-concat)
        (value/bounded-string!
         (apply str (mapv #(eval-expr % env functions fuel heap call-stack cap-call) args))
...
(defn execute
  "Executes one KIR export using normative typed-value semantics."
  ...
  ([kir function-name args {:keys [fuel cap-call pair-capacity]
   ...
   (let [functions (into {} (map (juxt :name identity) (:functions kir)))
         ...
         param-types (or (:param-types function)
                         (vec (repeat (count (:params function)) :i64)))]
     (when-not (and function (sequential? args) (= (count args) (count param-types)))
       (throw (ex-info "arguments do not match function arity" {:phase :ir :args args})))
     (doseq [[arg type] (map vector args param-types)]
       (validate-runtime-value! arg type {:function function-name :argument true}))
...
(defn lower [hir]
  (let [kernel-operations '#{kernel-load-u8 kernel-load-u8-4k kernel-load-u8-16k
        ...
        value (when (and (some? (:entry hir)) (= :i64 (:result hir))
