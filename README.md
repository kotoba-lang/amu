...
The same document also defines a ten-engine runtime comparison: Amu native,
Amu Wasm, Rust, Clojure, ClojureScript, Go, Mojo, CPython, and TypeScript on
both Node and Deno. Engines whose toolchain is absent are named in
`skippedEngines` rather than dropped. Its ratios are evidence for one declared
workload and host, not a universal language ranking.
...
Context ABI **v7** (2026-09-16) adds the four text heads as host slots:
`string-compare` 240, `string-fold-ascii` 248, `string-find-blank` 256,
`string-skip-blank` 264 — one memcmp, a one-byte-to-one-byte ASCII fold and
...
Context ABI **v10** (2026-09-16) adds two range operations that take no
view and mint none: `string-find-byte` 336 and `string-append-range` 344;
`examples/range-slots.kotoba` executes both under the loader.
