...
(deftest source-option-list-match-calls-and-matches-a-named-capability
  (let [descriptor [:option :vector-i64]
        ...
        "(ns component.match-capability
           ...
           (:capabilities #{:http/post}))
         (defn choose
           [value [:option :vector-i64] fallback :i64] :i64
           (match-option value [:option :vector-i64]
             (none fallback)
             (some items
               (match-option
                 (typed-cap-call
                   ...
                   [:option :vector-i64]
                   [:option :vector-i64]
                   (option-some-of [:option :vector-i64] items))
                 [:option :vector-i64]
                 (none fallback)
        ...
        closed (composition/compose-closed application [provider])
        path (Files/createTempFile
              "kotoba-source-match-aggregate-capability-" ".wasm"
              (make-array FileAttribute 0))]
    (try
      (Files/write path ^bytes (:bytes closed)
                   (make-array java.nio.file.OpenOption 0))
...
(deftest source-option-bool-list-match-calls-and-matches-a-named-capability
  (let [descriptor [:option [:list :bool]]
        source
        "(ns component.match-bool-list-capability
           (:export [choose echo])
           (:capabilities #{:http/post}))
         (defn choose
           [value [:option [:list :bool]] fallback :i64] :i64
           (match-option value [:option [:list :bool]]
             (none fallback)
             (some items
               (match-option
                 (typed-cap-call
                   ...
                   [:option [:list :bool]]
                   [:option [:list :bool]]
                   (option-some-of [:option [:list :bool]] items))
                 [:option [:list :bool]]
                 (none fallback)
                 (some returned (vector-count returned))))))
         (defn echo [value :i64] :i64 value)"
        ...
        closed (composition/compose-closed application [provider])
        path (Files/createTempFile
              "kotoba-source-match-bool-list-capability-" ".wasm"
              (make-array FileAttribute 0))]
    (try
      (Files/write path ^bytes (:bytes closed)
                   (make-array java.nio.file.OpenOption 0))
      ...
      (doseq [[invoke expected]
              [["choose(none, 9)" "9"]
               ["choose(some([]), 9)" "0"]
               ["choose(some([true, false, true]), 9)" "3"]
               ["echo(11)" "11"]]]
        (let [run (shell/sh wasmtime-binary "run" "--invoke"
                            invoke (str path))]
          (is (zero? (:exit run)) (:err run))
...
(deftest source-option-record-list-match-calls-and-matches-a-named-capability
  (let [descriptor [:option [:list [:ref :demo/match-point]]]
        schemas {:demo/match-point
                 [:record :demo/match-point [[:x :i64] [:visible :bool]]]}
        source
        "(ns component.match-record-list-capability
           (:export [choose echo])
           (:capabilities #{:http/post})
           (:schemas
            {:demo/match-point
             [:record :demo/match-point [[:x :i64] [:visible :bool]]]}))
         (defn choose
           [value [:option [:list [:ref :demo/match-point]]]
            fallback :i64] :i64
           (match-option value [:option [:list [:ref :demo/match-point]]]
             (none fallback)
             (some items
               (match-option
                 (typed-cap-call
                   :http/post
                   [:option [:list [:ref :demo/match-point]]]
                   [:option [:list [:ref :demo/match-point]]]
                   (option-some-of
                    [:option [:list [:ref :demo/match-point]]]
                    items))
        ...
        closed (composition/compose-closed application [provider])
        path (Files/createTempFile
              "kotoba-source-match-record-list-capability-" ".wasm"
              (make-array FileAttribute 0))]
    (try
      (Files/write path ^bytes (:bytes closed)
                   (make-array java.nio.file.OpenOption 0))
      ...
      (doseq [[invoke expected]
              [["choose(none, 9)" "9"]
               ["choose(some([]), 9)" "0"]
               [(str "choose(some([{x: 7, visible: true}, "
                     "{x: -2, visible: false}]), 9)")
                "2"]
               ["echo(11)" "11"]]]
        (let [run (shell/sh wasmtime-binary "run" "--invoke"
...
(deftest source-option-union-list-match-calls-and-matches-a-named-capability
  (let [descriptor [:option [:list [:result :string :bool]]]
        source
        "(ns component.match-union-list-capability
           (:export [choose echo])
           (:capabilities #{:http/post}))
         (defn choose
           [value [:option [:list [:result :string :bool]]]
            fallback :i64] :i64
           (match-option value [:option [:list [:result :string :bool]]]
             (none fallback)
             (some items
               (match-option
                 (typed-cap-call
                   :http/post
                   [:option [:list [:result :string :bool]]]
                   [:option [:list [:result :string :bool]]]
                   (option-some-of
                    [:option [:list [:result :string :bool]]]
                    items))
        ...
        closed (composition/compose-closed application [provider])
        path (Files/createTempFile
              "kotoba-source-match-union-list-capability-" ".wasm"
              (make-array FileAttribute 0))]
    (try
      (Files/write path ^bytes (:bytes closed)
                   (make-array java.nio.file.OpenOption 0))
      ...
      (doseq [[invoke expected]
              [["choose(none, 9)" "9"]
               ["choose(some([]), 9)" "0"]
               ["choose(some([ok(\"hello\"), err(true), ok(\"安全\")]), 9)"
                "3"]
               ["echo(11)" "11"]]]
        (let [run (shell/sh wasmtime-binary "run" "--invoke"
                            invoke (str path))]
...
(deftest source-option-f64-list-match-calls-and-matches-a-named-capability
  (let [descriptor [:option :vector-f64]
        source
        "(ns component.match-f64-capability
           (:export [choose echo])
           (:capabilities #{:http/post}))
         (defn choose
           [value [:option :vector-f64] fallback :i64] :i64
           (match-option value [:option :vector-f64]
             (none fallback)
             (some items
               (match-option
                 (typed-cap-call
                   ...
                   [:option :vector-f64]
                   [:option :vector-f64]
                   (option-some-of [:option :vector-f64] items))
                 [:option :vector-f64]
                 (none fallback)
                 (some returned (vector-f64-count returned))))))
         (defn echo [value :i64] :i64 value)"
        compiled (compiler/compile-source
                  source :wasm32-wasi-kotoba-v1
                  {:allow #{[:cap/call 4]}})
        ...
        closed (composition/compose-closed application [provider])
        path (Files/createTempFile
              "kotoba-source-match-f64-list-capability-" ".wasm"
              (make-array FileAttribute 0))]
    (try
      (Files/write path ^bytes (:bytes closed)
                   (make-array java.nio.file.OpenOption 0))
