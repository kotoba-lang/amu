...
(deftest typed-sets-are-unique-canonically-ordered-and-persistent
  ;; The set predicate in this fixture was called `contains?` until 2026-09-03.
  ;; kotoba-sema 1a073853 reserves that name -- it is an admitted operation
  ;; head, and a user `defn` shadowing one is refused with `reserved function
  ;; name`. Renamed to `has-item?`, which shadows nothing. Nothing about the
  ;; typed-set claims changes; the fixture had been borrowing a name the
  ;; language now owns.
  (let [type [:set :i64]
