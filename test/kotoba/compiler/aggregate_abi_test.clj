...
(deftest pinned-closure-carries-the-complete-native-boundary
  ...
  ;;
  ;; Advanced 2026-09-03 to the tip: `2cfa97ae` computes the i64 key order
  ;; without `clojure.core/compare`. That is the KIR half of kotoba-sema
  ;; 93790c4b (integer and string map keys through the friendly surface): the
  ;; frontend admitting `{3 30 1 10}` is worth nothing if the runtime cannot
  ;; order those keys portably. Also carries the four codebook dequant formats
