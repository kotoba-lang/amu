...
(deftest persistent-list-syntax-desugars-to-the-bounded-pair-arena
  ;; `empty?` is :bool-typed; tally with `(if p 1 0)` as Clojure does.
  (let [source "(defn sum2 [xs] (+ (first xs) (first (rest xs))))
                (defn main [] (+ (sum2 (list 20 22))
                                 (if (empty? (rest (cons 9 (list)))) 1 0)))"
        results (mapv #(compiler/compile-source source %) compiler/targets)
        kir (:kir (first results))
        printed (pr-str kir)]
...
(deftest safe-predicates-and-second-desugar-to-verified-core-operations
  ;; Predicates are :bool-typed (language profile 5), so a numeric tally is
  ;; written the way Clojure writes it -- `(if p 1 0)` -- not by summing
  ;; predicate results directly.
  (let [source "(defn classify [x] (+ (if (zero? x) 1 0) (if (pos? x) 1 0)
                                      (if (neg? x) 1 0) (if (not x) 1 0)
                                      (if (not= x 4) 1 0)))
