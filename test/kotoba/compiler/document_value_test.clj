...
(def vector-source
  "(ns document.vector (:export [main first-item kind doc-kind bool-kind key-name entry changed tail removed missing missing-entry same-doc same-sample different-sample signed-zero-equal repeated-nulls repeated-value bad-assoc bad-drop]))
   ...
   (defn repeated-nulls [] :document
     (document-map :a (document-null) :b (document-null)))
   (defn repeated-value [] :document
     (let [item (document-map :name (document-string \"same\"))]
  ...
  (let [wasm (compiler/compile-source vector-source :wasm32-browser-kotoba-v1)
        ...
        js-probe (script-probe
                  ...
                  (str "if(x['first-item']()!==1n||x.kind()!==':fixed'||x['bool-kind']()!==true||x['key-name']()!=='type'||x.missing()!==false||x['missing-entry']()!==false)process.exit(2);"
                       ...
                       "const ns=x['repeated-nulls'](),rv=x['repeated-value']();if(ns[1].length!==2||rv[1].length!==2||rv[1][0][1][0][1][1]!=='same'||rv[1][0]===rv[1][1])process.exit(11);"
                       ...
                       "const sharedChild=['null'],shared=['vector',[sharedChild,sharedChild]];let sharedRejected=false;try{x['doc-kind'](shared)}catch(e){sharedRejected=true}if(!sharedRejected)process.exit(12);"
        ...
        wasm-probe (node-probe
                    ...
                    (str "const x=h.instance.exports;if(x['first-item']()!==1n||x.kind()!==':fixed'||x['bool-kind']()!==true||x['key-name']()!=='type'||x.missing()!==false||x['missing-entry']()!==false)process.exit(2);"
                         ...
                         "const ns=x['repeated-nulls'](),rv=x['repeated-value']();if(ns[1].length!==2||rv[1].length!==2||rv[1][0][1][0][1][1]!=='same')process.exit(11);"
                         ...
                         "const sharedChild=['null'],shared=['vector',[sharedChild,sharedChild]];let sharedRejected=false;try{h.typedValues.document(shared)}catch(e){sharedRejected=true}if(!sharedRejected)process.exit(12);"
                         "for(const name of ['bad-assoc','bad-drop']){let rejected=false;try{x[name]()}catch(e){rejected=true}if(!rejected)process.exit(4)}"))]
