(ns kotoba.compiler.clock-native-kexe-oracle-test
  ...
  Production `:native-aot` remains `:aiueos-c-free-bare-metal-v1` (ADR 0271).
...
(defonce measured-runtime
  (delay
    (let [{:keys [runtime loader-bytes]} (executor/measure-runtime)
          loader (atomic-output/temp-file! "kotoba-clock-kexe-" "")]
...
(deftest clock-kit-keeps-native-aot-and-jit-pending
  "Pins only what THIS suite measures. The hosted kexe oracle below is
  evidence about :native-aot -- that a clock request now reaches a real
  process and returns a record instead of trapping in request validation --
  and it is deliberately NOT evidence that the flag should move: ADR 0271
  says a C loader is an oracle, never a production native-effect surface.

  It says nothing about :wasm-aot, which is a wasm component seam. An
  assertion about that key used to sit here; it was removed when the WASI 0.3
  suite was held back (see clock-v1.edn), because a test that pins a key it
  cannot measure fails for a reason it does not name."
