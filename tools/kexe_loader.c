 ...
 *   tools/kexe_loader_decisions.kotoba sha256 3c7d13562c7c873309050ea576e12c2dad90bcc852238e2dc7d3513d9b974142
 *   tools/kexe_loader_names.kotoba     sha256 b2420fbd630026f8b1470c546ecce9c99ee541244a6373bb465f8d96d03b8fc2
...
#define KEXE_TRAP_LINE_SIGILL 0u
#define KEXE_TRAP_LINE_SIGTRAP 1u
#define KEXE_TRAP_LINE_SIGFPE 2u
#define KEXE_TRAP_LINE_SIGBUS 3u
#define KEXE_TRAP_LINE_SIGSEGV 4u
#define KEXE_TRAP_LINE_SIGXCPU 5u
#define KEXE_TRAP_LINE_SIGALRM 6u
#define KEXE_TRAP_LINE_SIGSYS 7u
#define KEXE_TRAP_LINE_SIGNAL_UNKNOWN 8u
#define KEXE_TRAP_LINE_HOST_STACK_EXHAUSTED 9u
#define KEXE_TRAP_LINE_WALL_TIMEOUT 10u
#define KEXE_TRAP_LINE_UNHANDLED_CHILD_SIGNAL 11u
#define KEXE_TRAP_LINE_BUDGET_PAIRS 12u
#define KEXE_TRAP_LINE_BUDGET_VECTORS 13u
#define KEXE_TRAP_LINE_BUDGET_VECTOR_ITEMS 14u
#define KEXE_TRAP_LINE_BUDGET_STRING_POOL 15u
#define KEXE_TRAP_LINE_BUDGET_KGRAPH 16u
#define KEXE_TRAP_LINE_BUDGET_FUEL 17u
#define KEXE_TRAP_LINE_RESULT_STRING 18u
#define KEXE_TRAP_LINE_RESULT_RECORD 19u
#define KEXE_TRAP_LINE_RESULT_OPTION 20u
#define KEXE_TRAP_LINE_RESULT_RESULT 21u
#define KEXE_TRAP_LINE_RESULT_VARIANT 22u
#define KEXE_TRAP_LINE_ARENA_VECTOR_TABLE 23u
#define KEXE_TRAP_LINE_ARENA_VECTOR_ITEMS 24u
#define KEXE_TRAP_LINE_SANDBOX_FILESYSTEM 25u
#define KEXE_TRAP_LINE_SANDBOX_NETWORK 26u
#define KEXE_TRAP_LINE_SANDBOX_PROCESS 27u
#define KEXE_TRAP_LINE_VALUE_PREFIX 28u
#define KEXE_TRAP_LINE_COUNT 29u
static const char *const kexe_trap_lines[KEXE_TRAP_LINE_COUNT] = {
  ...
  "KEXE_TRAP {:kind :signal :signal :SIGFPE}\n",
  "KEXE_TRAP {:kind :signal :signal :SIGBUS}\n",
  "KEXE_TRAP {:kind :signal :signal :SIGSEGV}\n",
  "KEXE_TRAP {:kind :signal :signal :SIGXCPU}\n",
  "KEXE_TRAP {:kind :signal :signal :SIGALRM}\n",
  "KEXE_TRAP {:kind :signal :signal :SIGSYS}\n",
  "KEXE_TRAP {:kind :signal :signal :unknown}\n",
  ...
  "KEXE_TRAP {:kind :budget :reason :budget/cells :arena :pairs}\n",
  "KEXE_TRAP {:kind :budget :reason :budget/cells :arena :vectors}\n",
  "KEXE_TRAP {:kind :budget :reason :budget/cells :arena :vector-items}\n",
  "KEXE_TRAP {:kind :budget :reason :budget/bytes :arena :string-pool}\n",
  "KEXE_TRAP {:kind :budget :reason :budget/cells :arena :kgraph}\n",
  ...
  "KEXE_TRAP {:kind :result :reason :invalid-option-i64}\n",
  "KEXE_TRAP {:kind :result :reason :invalid-result-i64}\n",
  "KEXE_TRAP {:kind :result :reason :invalid-variant}\n",
  "KEXE_TRAP {:kind :arena :reason :vector-table-exhausted}\n",
  "KEXE_TRAP {:kind :arena :reason :vector-items-exhausted}\n",
  "KEXE_TRAP {:kind :sandbox :reason :filesystem-denied}\n",
  "KEXE_TRAP {:kind :sandbox :reason :network-denied}\n",
  "KEXE_TRAP {:kind :sandbox :reason :process-denied}\n",
  "KEXE_TRAP {:kind :value :reason :",
};
static const size_t kexe_trap_line_lengths[KEXE_TRAP_LINE_COUNT] = {
  42u, 43u, 42u, 42u, 43u, 43u, 43u, 42u, 43u, 54u, 52u, 62u, 62u, 64u, 69u, 68u, 63u, 47u, 57u, 56u, 54u, 54u, 51u, 57u, 57u, 54u, 51u, 51u, 33u
};

#define KEXE_VALUE_TRAP_COUNT 12u
static const char *const kexe_value_trap_names[KEXE_VALUE_TRAP_COUNT] = {
  NULL,
  "utf8/unexpected-continuation",
  "utf8/overlong",
  "utf8/invalid-byte",
  "utf8/truncated",
  ...
  "utf8/above-max",
  "string/too-large",
  "bytes/item-out-of-range",
  "bytes/too-large",
  "bytes/slice-bounds",
};

#define KEXE_RESULT_TYPE_COUNT 4u
...
#if defined(__aarch64__)
#define KEXE_DECISIONS_CAP_ADMITTED_OFFSET 1400u
#define KEXE_DECISIONS_DECIDE_OFFSET 3532u
static const uint8_t kexe_decisions_code[4060] = {
  ,
  ...
  ,
};
#endif
#if defined(__x86_64__)
#define KEXE_DECISIONS_CAP_ADMITTED_OFFSET 2232u
#define KEXE_DECISIONS_DECIDE_OFFSET 5526u
static const uint8_t kexe_decisions_code[6364] = {
  ,
  ...
  ,
};
#endif
/* KEXE-DECISIONS-GENERATED END */
 ...
 * them. */
/* KEXE_PAIR_CAPACITY: decided in tools/kexe_loader_decisions.kotoba; the value is in the KEXE-DECISIONS-GENERATED region. */
...
/* KEXE_PAIR_MAX, KEXE_KGRAPH_CAPACITY: decided in tools/kexe_loader_decisions.kotoba; the value is in the KEXE-DECISIONS-GENERATED region. */
/* Open arena scopes a guest may nest (ABI v6). A find that scopes each
 * directory descent nests as deep as the tree; 256 is the same figure the
 * verifier's expression depth allows. */
/* KEXE_ARENA_SCOPE_DEPTH: decided in tools/kexe_loader_decisions.kotoba; the value is in the KEXE-DECISIONS-GENERATED region. */
/* The string arena.
 *
...
/* KEXE_STRING_POOL_BYTES: decided in tools/kexe_loader_decisions.kotoba; the value is in the KEXE-DECISIONS-GENERATED region. */
 ...
 * trees away. */
/* KEXE_STRING_POOL_MAX: decided in tools/kexe_loader_decisions.kotoba; the value is in the KEXE-DECISIONS-GENERATED region. */
...
/* KEXE_REGION_CAPACITY, KEXE_REGION_POOL_BYTES: decided in tools/kexe_loader_decisions.kotoba; the value is in the KEXE-DECISIONS-GENERATED region. */
...
/* KEXE_RECORD_FIELD_LIMIT: decided in tools/kexe_loader_decisions.kotoba; the value is in the KEXE-DECISIONS-GENERATED region. */
...
/* KEXE_FUEL_MAX: decided in tools/kexe_loader_decisions.kotoba; the value is in the KEXE-DECISIONS-GENERATED region. */
...
/* KEXE_CPU_SECONDS, KEXE_WALL_SECONDS, KEXE_SECONDS_MAX: decided in tools/kexe_loader_decisions.kotoba; the value is in the KEXE-DECISIONS-GENERATED region. */
...
/* KEXE_VECTOR_CAPACITY, KEXE_VECTOR_ITEM_CAPACITY, KEXE_VECTOR_MAX, KEXE_VECTOR_ITEM_MAX, KEXE_VECTOR_ITEM_LIMIT: decided in tools/kexe_loader_decisions.kotoba; the value is in the KEXE-DECISIONS-GENERATED region. */
...
struct kexe_context_v11 {
   ...
   * its own (7 since the text slots below), which is the same
  ...
  /* ABI v6 (2026-09-16, superproject ADR-2609160044). The region reset
   * behind `(arena-scope body)`: ENTER pushes the four arenas' marks --
   * pair_used, string_pool_used, vector_used, vector_item_used -- and LEAVE
   * pops them, releasing every handle, byte and item allocated between.
   * The kgraph is NOT reset: its datoms hold i64 words only and an assert
   * is an observable fact.
   *
   * Why this is safe without the host knowing what the guest holds: the
   * body of a scope is typed :i64 / :bool / :f64 by the frontend AND
   * re-derived by kotoba-verifier, so no handle can be the scope's value;
   * every mutable structure a body can reach (vector-i64 elements, datoms)
   * holds numbers; the in-place appends (vector_conj, string_concat's tail
   * path) write PAST outer handles and leave their lengths alone; and the
   * pair the backends emit is structural -- no KIR head names enter or
   * leave -- so a callee cannot leave its caller's scope. What the host
   * still checks: a leave at depth 0 traps, and the mark stack is bounded
   * (KEXE_ARENA_SCOPE_DEPTH), so a runaway nest traps rather than writing
   * past it. The version is 6 for the same one-directional reason it
   * became 5: a v5 host has nothing at 224 / 232. */
  int64_t (*arena_enter)(struct kexe_context_v11 *);
  int64_t (*arena_leave)(struct kexe_context_v11 *);
  /* ABI v7 (2026-09-16). Four text operations the command binaries measured
   * to need on the host side: `string_compare` (byte order, -1/0/1 --
   * sort's merge spent one host call per code point comparing),
   * `string_fold_ascii` (A-Z to a-z into a new pool string, one byte to one
   * byte so offsets carry over -- grep -i folded each line in 26 passes),
   * `string_find_blank` / `string_skip_blank` (the first blank / non-blank
   * byte offset at or after FROM, a code-point boundary; -1 / the length
   * when none; blank = C isspace -- wc and awk walked words a code point
   * per call). Reference semantics: osaho kotoba.kir.value. Version 7 for
   * the same one-directional reason as every bump before it. */
  ...
  /* ABI v8 (2026-09-16). Two line operations the line-oriented commands
   * measured to need: `string_index_of_from` (the first occurrence of
   * NEEDLE in S at or after byte offset FROM, a code-point boundary in
   * [0, len]; ABSOLUTE offset, -1 when none -- every line walk had been
   * cutting a view, a handle, to search from an offset) and
   * `string_compare_lines` (byte order, -1/0/1, of the newline-terminated
   * line of A starting at I against the line of B starting at J; a line
   * ends at the first newline, excluded, or the end of its string, and the
   * byte length names the empty line -- sort's merge and uniq cut two
   * views per comparison). The second is the first four-argument slot:
   * kotoba-mir carries the fourth in r8 / x4, the fifth C parameter here.
   * Reference semantics: osaho kotoba.kir.value. Version 8 for the same
   * one-directional reason as every bump before it. */
  ...
  /* ABI v9 (2026-09-16, kotoba-native ADR 0084, owner decision). Six DATA
   * pointers, written once here before the guest starts and never again,
   * through which the compiler emits five handle operations IN LINE --
   * pair_first, pair_second (string_byte_length), vector_count, vector_at,
   * vector_assoc_in_place -- as the same range check the checked_* twin
   * makes (`(uint64)(handle - 1) < *used`, then `(uint64)index < length`),
   * one table read and one load or store, failing with udf / ud2 = SIGILL,
   * the signal raise(SIGILL) sends here. The slots above stay installed and
   * answer any caller that still calls them. A v8 loader has nothing at
   * these offsets, which is what makes the version bump necessary: code
   * compiled against v9 that read them on a v8 host would read past the
   * context. Part of the compiler-checked context-abi for exactly that
   * reason, unlike code_base / code_length below. */
  uint64_t *pair_used_pointer;
  struct kexe_pair_v1 *pairs_base;
  uint8_t *pair_validated_base;
  uint64_t *vector_used_pointer;
  struct kexe_vector_v1 *vectors_base;
  int64_t *vector_items_base;
  /* ABI v10 (2026-09-16). Two range operations that take no VIEW and mint
   * none: `string_find_byte` (the first offset at or after FROM, a boundary
   * in [0, len], holding the ASCII byte B -- B must be 0-127 so a match is
   * a boundary; -1 when none) and `string_append_range` (ACC followed by
   * bytes [START, END) of S, both boundaries, START <= END; the tail append
   * applies exactly as string_concat's does). Measured 2026-09-16: a 33 MB
   * `cut -f2` spent 44% of its time minting views for its searches and
   * appends -- a newline needle per search, a line view, a field view. */
  ...
  /* ABI v11 (2026-09-25, superproject adr-2609242330). `:bytes` as a guest
   * value, and the string constructor the language did not have. A bytes
   * VALUE is a vector handle whose items are 0..255 -- the type keeps it
   * apart from a vector-i64, the representation is shared -- so bytes-empty,
   * bytes-count and bytes-at ARE vector_new_empty / vector_count /
   * vector_at. These four are what the vector table cannot express:
   *
   ...
   * Version 11 for the same one-directional reason as every bump before
   * it: a v10 host has nothing at 352-376. */
  int64_t (*string_from_utf8)(struct kexe_context_v11 *, int64_t);
...
struct kexe_shared_v11 {
  ...
  /* Which VALUE trap a guest hit, written by the refusing host call just
   * before its SIGILL and read by the supervisor to name it (ABI v11: the
   * utf8, string/too-large and bytes refusals). 0 = none recorded. */
  uint64_t value_trap;
  uint64_t pair_used;
  ...
  uint64_t arena_scope_marks[KEXE_ARENA_SCOPE_DEPTH][4];
...
_Static_assert(offsetof(struct kexe_shared_v11, region_pool) +
                   KEXE_REGION_POOL_BYTES == sizeof(struct kexe_shared_v11),
...
static struct kexe_shared_v11 *kexe_shared_for_budget = NULL;
...
/* ABI v11: the named value traps. The index is what a host call records in
 * `value_trap`; the text is the trap's name, identical to osaho's reference
 * interpreter and the ESM prelude, so one program refused on three backends
 * is refused with one name. */
/* The names themselves (kexe_value_trap_names) are kexe_loader_names.kotoba's
 * value-trap-name, in the generated region. */
#define KEXE_VALUE_TRAP_STRING_TOO_LARGE 8u
#define KEXE_VALUE_TRAP_BYTES_ITEM_OUT_OF_RANGE 9u
#define KEXE_VALUE_TRAP_BYTES_TOO_LARGE 10u
#define KEXE_VALUE_TRAP_BYTES_SLICE_BOUNDS 11u
/* KEXE_STRING_VALUE_LIMIT, KEXE_BYTES_VALUE_LIMIT (kotoba.kir.value/string-value-byte-limit and bytes-value-byte-limit): decided in tools/kexe_loader_decisions.kotoba; the value is in the KEXE-DECISIONS-GENERATED region. */
static void note_value_trap(uint64_t code) {
  if (kexe_shared_for_budget != NULL && kexe_shared_for_budget->value_trap == 0)
    kexe_shared_for_budget->value_trap = code;
}
...
_Static_assert(offsetof(struct kexe_context_v11, kgraph_assert) == 80, "kgraph ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, kgraph_get) == 88, "kgraph ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, kgraph_count) == 96, "kgraph ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, kgraph_entity_at) == 104, "kgraph ABI drift");
...
_Static_assert(sizeof(((struct kexe_shared_v11 *)0)->pairs)
                   == KEXE_PAIR_MAX * sizeof(struct kexe_pair_v1),
               "pair arena size drift");
_Static_assert(sizeof(((struct kexe_shared_v11 *)0)->datoms) == 98304,
               "kgraph arena size drift");
_Static_assert(offsetof(struct kexe_context_v11, vector_new_empty) == 152, "vector ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, vector_conj) == 160, "vector ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, vector_count) == 168, "vector ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, vector_at) == 176, "vector ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, vector_assoc) == 184, "vector ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, vector_drop) == 192, "vector ABI drift");
...
_Static_assert(offsetof(struct kexe_context_v11, string_compare) == 240, "text ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, string_fold_ascii) == 248, "text ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, string_find_blank) == 256, "text ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, string_skip_blank) == 264, "text ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, string_index_of_from) == 272, "line ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, string_compare_lines) == 280, "line ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, pair_used_pointer) == 288, "inline ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, pairs_base) == 296, "inline ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, pair_validated_base) == 304, "inline ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, vector_used_pointer) == 312, "inline ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, vectors_base) == 320, "inline ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, vector_items_base) == 328, "inline ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, string_find_byte) == 336, "range ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, string_append_range) == 344, "range ABI drift");
_Static_assert(offsetof(struct kexe_context_v11, string_from_utf8) == 352, "bytes ABI drift");
...
_Static_assert(sizeof(((struct kexe_shared_v11 *)0)->vector_items) == KEXE_VECTOR_ITEM_MAX * 8u,
               "vector item arena size drift");
_Static_assert(KEXE_VECTOR_ITEM_LIMIT <= KEXE_VECTOR_ITEM_MAX, "a vector must fit the arena");

/* The loader's DECISIONS, called (superproject adr-2609251400 C1, kind (b)).
 *
 * `kexe_decisions_code` is tools/kexe_loader_decisions.kotoba compiled by amu
 * for THIS ISA (the generated region above). It is mapped once, read+execute,
 * the way the guest is -- W^X, never writable and executable at once -- and
 * called through the same native kexe ABI the guest entry is (five i64
 * arguments; the context in r9 / x7). The context is PRIVATE to the
 * decisions: a decision charges the guest no fuel and no arena cell, so a
 * report is byte-identical to what it was when these were C.
 *
 * Mapped in main() before the fork and the sandbox (the sandboxed child has
 * no mmap); the lazy path is for harnesses that drive the checked_*
 * callbacks without main (tools/kexe_parser_fuzz.c).
 *
 * Nothing in this block decides anything. What each op means is in the
 * Kotoba source, next to the op number `decide` dispatches on; the numbers
 * below are that ABI, and scripts/test-loader-decisions.cljk runs every op
 * against the C it replaced. */
#if !defined(__aarch64__) && !defined(__x86_64__)
#error "no compiled loader decisions for this ISA: aarch64 and x86_64 only"
#endif
#define KEXE_DECIDE_CAP_PROVIDER 0
#define KEXE_DECIDE_TYPED_KINDS 1
#define KEXE_DECIDE_RECORD_FIELDS 2
#define KEXE_DECIDE_VARIANT_PROFILE 3
#define KEXE_DECIDE_VARIANT_ARG 4
#define KEXE_DECIDE_VARIANT_RESULT 5
#define KEXE_DECIDE_TAGGED 6
#define KEXE_DECIDE_BUDGET 7
#define KEXE_DECIDE_SCOPE 8
#define KEXE_DECIDE_WIRE_ID 9
/* decide op 7's budget numbers and verdicts. */
#define KEXE_BUDGET_ID_FUEL 0
#define KEXE_BUDGET_ID_STRING_POOL 1
#define KEXE_BUDGET_ID_PAIRS 2
#define KEXE_BUDGET_ID_VECTORS 3
#define KEXE_BUDGET_ID_VECTOR_ITEMS 4
#define KEXE_BUDGET_ID_CPU_SECONDS 5
#define KEXE_BUDGET_ID_WALL_SECONDS 6
#define KEXE_BUDGET_VERDICT_ZERO 1
#define KEXE_BUDGET_VERDICT_ABOVE_CEILING 2

static void fail(const char *message);
static struct kexe_context_v11 kexe_decision_context;
static const uint8_t *kexe_decision_code = NULL;

static void kexe_decisions_init(void) {
  if (kexe_decision_code != NULL) return;
  long pagesize = sysconf(_SC_PAGESIZE);
  size_t length = sizeof(kexe_decisions_code);
  size_t mapped = (length + (size_t)pagesize - 1) & ~((size_t)pagesize - 1);
  void *memory = mmap(NULL, mapped, PROT_READ | PROT_WRITE,
                      MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (memory == MAP_FAILED) fail("mmap decisions RW");
  memcpy(memory, kexe_decisions_code, length);
  if (mprotect(memory, mapped, PROT_READ | PROT_EXEC) != 0) fail("mprotect decisions RX");
  __builtin___clear_cache((char *)memory, (char *)memory + length);
  kexe_decision_context.version = 11;
  kexe_decision_code = (const uint8_t *)memory;
}

static int64_t kexe_decision_call(uint64_t offset, int64_t a, int64_t b,
                                  int64_t c, int64_t d, int64_t e) {
  kexe_decisions_init();
  kexe_decision_context.fuel = KEXE_FUEL_MAX;
  int64_t context = (int64_t)(uintptr_t)&kexe_decision_context;
#if defined(__x86_64__)
  kexe_fn6 fn = (kexe_fn6)(uintptr_t)(kexe_decision_code + offset);
  return fn(a, b, c, d, e, context);
#else
  kexe_fn8 fn = (kexe_fn8)(uintptr_t)(kexe_decision_code + offset);
  return fn(a, b, c, d, e, 0, 0, context);
#endif
}

static int64_t kexe_decide(int64_t op, int64_t a, int64_t b, int64_t c, int64_t d) {
  return kexe_decision_call(KEXE_DECISIONS_DECIDE_OFFSET, op, a, b, c, d);
}

/* The allow-list: `cap-admitted` reads the four mask words itself, so no
 * index into ALLOW is computed here. */
static int kexe_cap_admitted(const uint64_t allow[4], uint64_t id) {
  return kexe_decision_call(KEXE_DECISIONS_CAP_ADMITTED_OFFSET,
                            (int64_t)allow[0], (int64_t)allow[1],
                            (int64_t)allow[2], (int64_t)allow[3],
                            (int64_t)id) == 1;
}

/* Whether TEXT is one of the plain result type names (generated table). */
static int kexe_result_type_named(const char *text) {
  for (size_t i = 0; i < KEXE_RESULT_TYPE_COUNT; i++)
    if (strcmp(text, kexe_result_type_names[i]) == 0) return 1;
  return 0;
}

static void write_trap_line(unsigned line) {
  ssize_t written = write(STDERR_FILENO, kexe_trap_lines[line], kexe_trap_line_lengths[line]);
  (void)written;
}
...
static int64_t checked_cap_call(struct kexe_context_v11 *context,
  ...
  /* The qualification host's stub answer, value + 1, WRAPPING -- the i64 add
   * every Kotoba backend defines. `value + 1` was signed overflow, undefined
   * behaviour, at INT64_MAX, which the sanitized fuzz arm reported in CI
   * (superproject adr-2609251400 C1); INT64_MIN is what the compiled add
   * produced on both ISAs, and is now the answer by definition. */
...
static struct kexe_vector_v1 *resolve_vector(struct kexe_shared_v11 *shared,
                                             int64_t handle) {
...
static void arena_exhausted(uint64_t budget) {
  note_budget(budget);
  write_trap_line(budget == KEXE_BUDGET_CELLS_VECTOR_ITEMS
                      ? KEXE_TRAP_LINE_ARENA_VECTOR_ITEMS
...
static int64_t intern_vector(struct kexe_shared_v11 *shared,
...
static int kgraph_entity_seen_before(const struct kexe_shared_v11 *shared,
...
static const uint8_t *resolve_string_bytes(struct kexe_context_v11 *context,
   ...
   * The range must lie within the bytes that EXIST (string_pool_used), not
   * merely within the budget: a pair a guest builds by hand can name any
   * offset and length, and one reaching past the used bytes into the
   * unwritten pool used to resolve -- found 2026-09-16 by the sanitized
   * fuzz arm as a memcpy whose source overlapped the concat's destination
   * (the next free bytes). Refused here, once, for every string slot. */
  uint64_t pool_offset = (uint64_t)(-(offset + 1));
  if (pool_offset + (uint64_t)length > shared->string_pool_used ||
...
static int allocate_host_pair(struct kexe_shared_v11 *shared,
...
static int parse_variant_profile(const char *text, uint64_t *case_count,
  ...
  if (sscanf(text, "variant:%llu:%llu%n", &cases, &mask, &consumed) != 2 ||
      text[consumed] != '\0' ||
      kexe_decide(KEXE_DECIDE_VARIANT_PROFILE, (int64_t)cases, (int64_t)mask, 0, 0) != 1)
    return 0;
 ...
 * working. */
static int parse_guest_arg(struct kexe_shared_v11 *shared,
  ...
  if (strncmp(text, "v:", 2) == 0) {
    ...
    if (sscanf(text, "v:%llu:%llu:%c:%lld%n", &cases, &ordinal, &kind,
        ...
        kexe_decide(KEXE_DECIDE_VARIANT_ARG, (int64_t)cases, (int64_t)ordinal,
                    (int64_t)(unsigned char)kind, (int64_t)payload) != 1) return -1;
...
static const uint8_t *inspect_string_result(const struct kexe_shared_v11 *shared,
...
static int inspect_record_result(const struct kexe_shared_v11 *shared,
                                 int64_t handle, uint64_t field_count,
                                 int64_t fields[KEXE_RECORD_FIELD_LIMIT]) {
  /* The rule is decide op 2's; the second clause is FIELDS' array bound. */
  if (kexe_decide(KEXE_DECIDE_RECORD_FIELDS, (int64_t)field_count, 0, 0, 0) != 1 ||
      field_count > KEXE_RECORD_FIELD_LIMIT) return 0;
...
static int inspect_tagged_i64_result(const struct kexe_shared_v11 *shared,
                                     int64_t handle, int option,
                                     int64_t *tag, int64_t *payload) {
  if (handle <= 0 || (uint64_t)handle > shared->pair_used) return 0;
  const struct kexe_pair_v1 *pair = &shared->pairs[(uint64_t)handle - 1u];
  if (kexe_decide(KEXE_DECIDE_TAGGED, option ? 1 : 0, pair->first, pair->second, 0) != 1)
    return 0;
  *tag = pair->first;
  *payload = pair->second;
  return 1;
...
static int inspect_variant_result(const struct kexe_shared_v11 *shared,
                                  int64_t handle, uint64_t case_count,
                                  uint64_t bool_mask, int64_t *ordinal,
                                  int64_t *payload) {
  if (handle <= 0 || (uint64_t)handle > shared->pair_used) return 0;
  const struct kexe_pair_v1 *pair = &shared->pairs[(uint64_t)handle - 1u];
  if (kexe_decide(KEXE_DECIDE_VARIANT_RESULT, (int64_t)case_count, (int64_t)bool_mask,
                  pair->first, pair->second) != 1) return 0;
  *ordinal = pair->first;
  *payload = pair->second;
  return 1;
}
...
static int peek_vector(struct kexe_context_v11 *context, int64_t handle,
...
static int valid_string_handle(struct kexe_context_v11 *context, int64_t value) {
...
static int read_string_handle(struct kexe_context_v11 *context, int64_t value,
...
static int64_t intern_utf8(struct kexe_context_v11 *context,
...
static int64_t gpu_compute_provider(struct kexe_context_v11 *context, int64_t request) {
...
static int valid_dataspace_request(struct kexe_context_v11 *context,
                                   int64_t value) {
...
static int valid_dataspace_result(struct kexe_context_v11 *context,
                                  int64_t value) {
  int64_t ordinal, payload;
  if (!peek_pair(context, value, 0, &ordinal) ||
      !peek_pair(context, value, 1, &payload)) return 0;
...
static int valid_ui_node(struct kexe_context_v11 *context, int64_t node) {
...
static int valid_ui_nodes(struct kexe_context_v11 *context, int64_t nodes) {
  uint64_t length = 0, i;
...
static int valid_ui_commit_request(struct kexe_context_v11 *context,
                                   int64_t value) {
...
static int valid_ui_commit_result(struct kexe_context_v11 *context,
                                  int64_t value) {
...
static int valid_ui_event_request(struct kexe_context_v11 *context,
                                  int64_t value) {
  int64_t after, tail;
...
static int valid_ui_event(struct kexe_context_v11 *context, int64_t value) {
...
static int valid_ui_event_result(struct kexe_context_v11 *context,
                                 int64_t value) {
  int64_t tag, payload;
...
static int64_t intern_pool_string(struct kexe_context_v11 *context,
                                  const char *text) {
  struct kexe_shared_v11 *shared = (struct kexe_shared_v11 *)context;
...
static int valid_clock_request(const struct kexe_shared_v11 *shared, int64_t value,
...
static int valid_clock_result(const struct kexe_shared_v11 *shared, int64_t value) {
...
static int64_t clock_error_result(struct kexe_context_v11 *context,
...
static int64_t clock_success_result(struct kexe_context_v11 *context,
...
static int64_t hosted_clock_v1(struct kexe_context_v11 *context, int64_t request) {
  struct kexe_shared_v11 *shared = (struct kexe_shared_v11 *)context;
...
static int valid_typed_value(struct kexe_context_v11 *context,
  ...
  if (kind == KEXE_TYPED_OPTION_I64 || kind == KEXE_TYPED_RESULT_I64) {
    ...
    return kexe_decide(KEXE_DECIDE_TAGGED, kind == KEXE_TYPED_OPTION_I64 ? 1 : 0,
                       tag, payload, 0) == 1;
  }
...
static int64_t ds_result_facet(struct kexe_context_v11 *context, int64_t id) {
  return checked_pair_new(context, DS_RES_FACET,
                          checked_pair_new(context, id, 0));
}

static int64_t ds_result_retracted(struct kexe_context_v11 *context,
                                   int64_t count) {
  return checked_pair_new(context, DS_RES_RETRACTED,
                          checked_pair_new(context, count, 0));
}

static int64_t ds_result_asserted(struct kexe_context_v11 *context,
...
static int64_t ds_result_matches(struct kexe_context_v11 *context,
...
static int64_t ds_notice_handle(struct kexe_context_v11 *context, int kind) {
  const char *text = kind == 1 ? ds_retract_notice : ds_assert_notice;
  return intern_utf8(context, (const uint8_t *)text, strlen(text));
}

static int64_t ds_empty_handle(struct kexe_context_v11 *context) {
  return intern_utf8(context, (const uint8_t *)ds_empty, strlen(ds_empty));
}

static int64_t dataspace_inject(struct kexe_context_v11 *context,
                                int64_t request) {
...
static int64_t ui_commit_inject(struct kexe_context_v11 *context,
                                int64_t request) {
  int64_t base_rev, rest, nodes, tail;
  uint64_t length = 0;
  const int64_t *items = NULL;
...
static int64_t ui_event_inject(struct kexe_context_v11 *context,
                               int64_t request) {
  int64_t after, tail;
...
static int64_t env_read_provider(struct kexe_context_v11 *context,
                                 int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
...
/* VALUE as unsigned decimal into OUT (at least 21 bytes); the digit count. */
static size_t kexe_decimal(char *out, uint64_t value) {
  char tmp[24];
  size_t n = 0;
...
static int64_t io_write_provider(struct kexe_context_v11 *context,
                                 int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
  if (!read_string_handle(context, request, &bytes, &length)) {
  ...
  /* Decimal, no sign, no padding: at most 20 digits for a uint64. Written
   * by hand rather than snprintf (2026-09-16): a write is one capability
   * call per line in every line-oriented command, and the formatting was
   * about a third of the 100 ns each cost. */
  char count[24];
  size_t digits = kexe_decimal(count, length);
  return intern_utf8(context, (const uint8_t *)count, digits);
}
...
static int64_t io_write_error_provider(struct kexe_context_v11 *context,
                                       int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
  if (!read_string_handle(context, request, &bytes, &length)) {
...
static int64_t io_read_provider(struct kexe_context_v11 *context,
                                int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
  if (!read_string_handle(context, request, &bytes, &length)) {
...
static int64_t clock_text_provider(struct kexe_context_v11 *context,
                                   int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
  if (!read_string_handle(context, request, &bytes, &length)) {
...
/* wire id 3 = :hash/sha256. The request bytes' SHA-256, as 64 lowercase
 * hex characters. Pure: it crosses no boundary and carries no scope, which
 * is why the catalog classifies it :public -- the grant bit is what stands
 * between a guest and the computation, and nothing else needs to.
 *
 * Until 2026-09-16 the switch below fell through to identity for this id,
 * so a guest asking for a digest got its input back (superproject
 * ADR-2609161710: `shasum -a 256` is 612 of 1,268,018 agent Bash calls).
 * FIPS 180-4, the straightforward 32-bit schedule; the test vectors in
 * amu's test-package-command are "abc" and the empty string. */
static const uint32_t kexe_sha256_k[64] = {
...
static inline uint32_t kexe_rotr32(uint32_t x, unsigned n) { return (x >> n) | (x << (32u - n)); }

static void kexe_sha256_block(uint32_t h[8], const uint8_t block[64]) {
  uint32_t w[64];
  for (int i = 0; i < 16; i++) {
  ...
  for (int i = 16; i < 64; i++) {
    uint32_t s0 = kexe_rotr32(w[i - 15], 7) ^ kexe_rotr32(w[i - 15], 18) ^ (w[i - 15] >> 3);
    uint32_t s1 = kexe_rotr32(w[i - 2], 17) ^ kexe_rotr32(w[i - 2], 19) ^ (w[i - 2] >> 10);
    w[i] = w[i - 16] + s0 + w[i - 7] + s1;
  }
  uint32_t a = h[0], b = h[1], c = h[2], d = h[3], e = h[4], f = h[5], g = h[6], hh = h[7];
  for (int i = 0; i < 64; i++) {
    uint32_t t1 = hh + (kexe_rotr32(e, 6) ^ kexe_rotr32(e, 11) ^ kexe_rotr32(e, 25)) +
                  ((e & f) ^ (~e & g)) + kexe_sha256_k[i] + w[i];
    uint32_t t2 = (kexe_rotr32(a, 2) ^ kexe_rotr32(a, 13) ^ kexe_rotr32(a, 22)) +
                  ((a & b) ^ (a & c) ^ (b & c));
...
static int64_t hash_sha256_provider(struct kexe_context_v11 *context, int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
  if (!read_string_handle(context, request, &bytes, &length)) {
    raise(SIGILL);
  ...
  while (length - done >= 64) {
    kexe_sha256_block(h, bytes + done);
    done += 64;
  }
  uint8_t tail[128];
  size_t rest = (size_t)(length - done);
  memcpy(tail, bytes + done, rest);
  tail[rest] = 0x80u;
  size_t padded = rest + 1 <= 56 ? 64 : 128;
  memset(tail + rest + 1, 0, padded - rest - 1);
  uint64_t bits = length * 8u;
  for (int i = 0; i < 8; i++) tail[padded - 1 - i] = (uint8_t)(bits >> (8 * i));
  kexe_sha256_block(h, tail);
  if (padded == 128) kexe_sha256_block(h, tail + 64);
  char hex[64];
  static const char digits[] = "0123456789abcdef";
  for (int i = 0; i < 8; i++) {
    for (int j = 0; j < 4; j++) {
      uint8_t byte = (uint8_t)(h[i] >> (24 - 8 * j));
      hex[8 * i + 2 * j] = digits[byte >> 4];
      hex[8 * i + 2 * j + 1] = digits[byte & 0x0fu];
    }
  }
...
static const uint8_t *granted_span(struct kexe_shared_v11 *shared,
...
static int64_t identity_verify_provider(struct kexe_context_v11 *context, int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
  uint64_t a[4];
  if (!read_string_handle(context, request, &bytes, &length) ||
...
static int64_t cli_args_provider(struct kexe_context_v11 *context,
                                 int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
  if (!read_string_handle(context, request, &bytes, &length)) {
...
/* KEXE_SCOPE_ENTRIES: decided in tools/kexe_loader_decisions.kotoba; the value is in the KEXE-DECISIONS-GENERATED region. */
...
static int kexe_scope_admit(const struct kexe_scope *scope, const char *target,
  ...
  for (int s = 0; s < scope->count; s++) {
    for (int v = 0; v < 2; v++) {
      ...
      /* Measured here, RULED by decide op 8 (scope-admitted): is the target
       * the entry or beneath it, and does its re-spelling fit? The byte
       * after a matching prefix is read only when the prefix matched. The
       * last clause is CANDIDATE's buffer bound, not the rule. */
      int prefix_equal = strncmp(target, base, base_length) == 0;
      const char *suffix = prefix_equal ? target + base_length : "";
      size_t suffix_length = strlen(suffix);
      size_t real_length = strlen(scope->resolved[s]);
      if (kexe_decide(KEXE_DECIDE_SCOPE, prefix_equal,
                      prefix_equal ? (int64_t)(unsigned char)target[base_length] : 0,
                      (int64_t)(real_length + suffix_length), 0) != 1 ||
          real_length + suffix_length >= 4096) continue;
...
static int kexe_scope_contains_fd(const struct kexe_scope *scope, int fd,
  ...
  for (int s = 0; s < scope->count; s++) {
    ...
    size_t base_length = strlen(base);
    int prefix_equal = actual_length >= base_length &&
                       memcmp(actual, base, base_length) == 0;
    if (kexe_decide(KEXE_DECIDE_SCOPE, prefix_equal,
                    prefix_equal ? (int64_t)(unsigned char)actual[base_length] : 0,
                    0, 0) == 1) return 1;
  }
  return 0;
#else
...
/* memmem, one definition on every platform rather than libc's where it
 * exists: the answer must not depend on the host's libc. A memchr for the
 * needle's first byte, then one memcmp per candidate -- the same shape the
 * Windows loader uses. The shape it replaced (2026-09-16) called memcmp at
 * EVERY haystack offset, a function call per byte; sampled at 54% of a
 * 33 MB sort, whose merge asks for the next newline once per line per
 * level, and at the same share of grep's line walk. */
static void *shim_memmem(const void *hay, size_t hlen, const void *needle, size_t nlen) {
  ...
  const uint8_t *end = h + (hlen - nlen);
  for (const uint8_t *p = h; p <= end; p++) {
    p = (const uint8_t *)memchr(p, n[0], (size_t)(end - p) + 1);
    if (p == NULL) return NULL;
    if (nlen == 1 || memcmp(p + 1, n + 1, nlen - 1) == 0) return (void *)p;
  }
  return NULL;
}
...
static int64_t fs_app_data_exists_provider(struct kexe_context_v11 *context,
                                           int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
...
static int64_t fs_app_data_read_provider(struct kexe_context_v11 *context,
                                         int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
  char target[4096], candidate[4096];
...
static int64_t fs_app_data_write_provider(struct kexe_context_v11 *context,
...
static int64_t kexe_answer_bool(struct kexe_context_v11 *context, int ok) {
...
static int64_t fs_app_data_stat_provider(struct kexe_context_v11 *context,
                                         int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
...
static int64_t fs_app_data_mtime_provider(struct kexe_context_v11 *context,
                                          int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
...
static int64_t fs_app_data_chmod_provider(struct kexe_context_v11 *context,
                                          int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
...
static int64_t fs_app_data_mkdir_provider(struct kexe_context_v11 *context,
                                          int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
...
static int64_t fs_app_data_unlink_provider(struct kexe_context_v11 *context,
                                           int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
...
static int64_t fs_app_data_rmdir_provider(struct kexe_context_v11 *context,
                                          int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
...
static int64_t fs_app_data_rename_provider(struct kexe_context_v11 *context,
                                           int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
...
static int64_t fs_app_data_range_read_provider(struct kexe_context_v11 *context,
                                               int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
  if (!read_string_handle(context, request, &bytes, &length) || bytes == NULL) {
...
static int64_t fs_browse_provider(struct kexe_context_v11 *context,
                                  int64_t request) {
  const uint8_t *bytes = NULL;
  uint64_t length = 0;
  char target[4096], candidate[4096];
...
static int64_t checked_typed_cap_call(struct kexe_context_v11 *context,
  ...
  int64_t result;
  /* WHICH provider answers (wire id x typed kind) is decide op 0
   * (kexe_loader_decisions.kotoba cap-provider, whose ids are the capability
   * catalog's :compiler-wire-id). The switch only calls what it is told. */
  switch (kexe_decide(KEXE_DECIDE_CAP_PROVIDER, (int64_t)id, (int64_t)request_kind, 0, 0)) {
  case 1:
  ...
  case 9:
    ...
    result = hash_sha256_provider(context, request);
    break;
  case 10:
...
/* ABI v8 line operations: see the slots' comment in the context struct. */
static int64_t checked_string_index_of_from(struct kexe_context_v11 *context,
  ...
  int64_t offset_n = checked_pair_get(context, needle, 0);
  int64_t length_n = checked_pair_get(context, needle, 1);
  if (length_h < 0 || length_n <= 0 || from < 0 || from > length_h) {
    raise(SIGILL);
    return 0;
  }
  const uint8_t *h = resolve_string_bytes(context, offset_h, length_h);
  ...
  /* FROM must be a code-point boundary: a continuation byte there is refused,
   * as string_substring refuses a view cut there. */
  if (from < length_h && (h[from] & 0xc0) == 0x80) { raise(SIGILL); return 0; }
  if (length_n > length_h - from) return -1;
  const uint8_t *found = (const uint8_t *)memmem(h + from, (size_t)(length_h - from),
                                                 n, (size_t)length_n);
  return found == NULL ? -1 : (int64_t)(found - h);
}
...
static int64_t checked_string_compare_lines(struct kexe_context_v11 *context,
  ...
  const uint8_t *a = resolve_string_bytes(context, offset_a, length_a);
  if (a == NULL || !ensure_valid_string(context, handle_a, a, length_a)) {
    raise(SIGILL);
    return 0;
  }
  /* Two lines of ONE string -- a sort's or uniq's whole case -- resolve and
   * validate that string once; the second handle's checks are the first's. */
  int64_t offset_b = offset_a, length_b = length_a;
  const uint8_t *b = a;
  if (handle_b != handle_a) {
    offset_b = checked_pair_get(context, handle_b, 0);
    length_b = checked_pair_get(context, handle_b, 1);
    if (length_b < 0) { raise(SIGILL); return 0; }
    b = resolve_string_bytes(context, offset_b, length_b);
    if (b == NULL || !ensure_valid_string(context, handle_b, b, length_b)) {
      raise(SIGILL);
      return 0;
    }
  }
  if (j < 0 || j > length_b) { raise(SIGILL); return 0; }
...
static int64_t checked_string_find_byte(struct kexe_context_v11 *context,
  ...
  if (bytes == NULL || !ensure_valid_string(context, handle, bytes, length)) {
    raise(SIGILL);
    return 0;
  }
  if (from < length && (bytes[from] & 0xc0) == 0x80) { raise(SIGILL); return 0; }
  if (from == length) return -1;
  const uint8_t *hit = (const uint8_t *)memchr(bytes + from, (int)byte, (size_t)(length - from));
  return hit == NULL ? -1 : (int64_t)(hit - bytes);
}

static int64_t checked_string_append_range(struct kexe_context_v11 *context,
  ...
  int64_t length_s = checked_pair_get(context, handle, 1);
  if (length_a < 0 || length_s < 0 || start < 0 || end < start || end > length_s ||
      length_a > INT64_MAX - (end - start)) {
    raise(SIGILL);
    return 0;
  }
  const uint8_t *a = resolve_string_bytes(context, offset_a, length_a);
  const uint8_t *b = resolve_string_bytes(context, offset_s, length_s);
  if (a == NULL || b == NULL ||
      !ensure_valid_string(context, acc, a, length_a) ||
      !ensure_valid_string(context, handle, b, length_s)) {
    raise(SIGILL);
    return 0;
  }
  if (start < length_s && (b[start] & 0xc0) == 0x80) { raise(SIGILL); return 0; }
  if (end < length_s && (b[end] & 0xc0) == 0x80) { raise(SIGILL); return 0; }
  int64_t length_b = end - start;
  const uint8_t *piece = b + start;
  int64_t total = length_a + length_b;
  uint64_t pool_offset = shared->string_pool_used;
  /* The same tail append as string_concat: A at the pool's end grows by the
   * piece; otherwise both are copied. The piece is read before the copy
   * moves the tail, so a range of A itself is safe. */
  ...
  if (a_is_tail) {
    ...
    memmove(shared->string_pool + shared->string_pool_used, piece, (size_t)length_b);
    ...
    memmove(shared->string_pool + pool_offset, a, (size_t)length_a);
    memmove(shared->string_pool + pool_offset + (uint64_t)length_a, piece, (size_t)length_b);
    shared->string_pool_used += (uint64_t)total;
  }
  int64_t result = checked_pair_new(context, -((int64_t)pool_offset) - 1, total);
  /* A code-point-bounded range of a valid string appended to a valid string. */
  return mark_validated(context, result);
}

/* ABI v11: see the four slots' comment in the context struct. */

/* The FIRST reason BYTES are not canonical UTF-8, as a value-trap index, or 0.
 * The same walk, in the same order, as osaho kotoba.kir.value/utf8-invalid-at
 * -- which is what makes "the same name on every backend" checkable: the
 * second byte's range is decided before the rest is looked for, so E0 80 is
 * overlong, not truncated. */
static uint64_t utf8_invalid_reason(const int64_t *items, uint64_t n) {
  uint64_t i = 0;
  while (i < n) {
    int64_t b0 = items[i];
    ...
    if (b0 > 0xf4) return 3;
    if (i + 1 >= n) return 4;
    int64_t b1 = items[i + 1];
    uint64_t k = b0 < 0xe0 ? 1 : b0 < 0xf0 ? 2 : 3;
    if ((b1 & 0xc0) != 0x80) return 5;
    if (b0 == 0xe0 && b1 < 0xa0) return 2;
    if (b0 == 0xf0 && b1 < 0x90) return 2;
    if (b0 == 0xed && b1 >= 0xa0) return 6;
    if (b0 == 0xf4 && b1 >= 0x90) return 7;
    for (uint64_t j = 2; j <= k; j++) {
      if (i + j >= n) return 4;
      if ((items[i + j] & 0xc0) != 0x80) return 5;
    }
    i += k + 1;
  }
  return 0;
...
static int64_t checked_string_from_utf8(struct kexe_context_v11 *context,
  ...
  const int64_t *items = &shared->vector_items[vector->offset];
  if (n > KEXE_STRING_VALUE_LIMIT) {
    note_value_trap(KEXE_VALUE_TRAP_STRING_TOO_LARGE);
    raise(SIGILL);
    return 0;
  }
  /* A bytes value's items are 0..255 by construction (bytes_from_vector);
   * a word outside that range means the handle is not a bytes value, and the
   * walk below would read it as a byte it is not. Refused, unnamed: a
   * malformed call, not a value the language can hold. */
  for (uint64_t i = 0; i < n; i++) {
    if (items[i] < 0 || items[i] > 255) { raise(SIGILL); return 0; }
  }
  uint64_t reason = utf8_invalid_reason(items, n);
  if (reason != 0) { note_value_trap(reason); raise(SIGILL); return 0; }
  if (shared->string_pool_used + n > kexe_string_pool_budget ||
      shared->string_pool_used + n < shared->string_pool_used) {
    note_budget(KEXE_BUDGET_BYTES_STRING_POOL);
    raise(SIGILL);
    return 0;
  }
  uint64_t pool_offset = shared->string_pool_used;
  for (uint64_t i = 0; i < n; i++) shared->string_pool[pool_offset + i] = (uint8_t)items[i];
  shared->string_pool_used += n;
  /* Validated above, by the stricter rule (named reasons) than valid_utf8's. */
  return mark_validated(context,
                        checked_pair_new(context, -((int64_t)pool_offset) - 1, (int64_t)n));
}
...
static int64_t checked_bytes_from_vector(struct kexe_context_v11 *context,
  ...
  uint64_t offset = vector->offset;
  uint64_t n = vector->length;
  if (n > KEXE_BYTES_VALUE_LIMIT) {
    note_value_trap(KEXE_VALUE_TRAP_BYTES_TOO_LARGE);
    raise(SIGILL);
    return 0;
  }
  for (uint64_t i = 0; i < n; i++) {
    int64_t item = shared->vector_items[offset + i];
    if (item < 0 || item > 255) {
      note_value_trap(KEXE_VALUE_TRAP_BYTES_ITEM_OUT_OF_RANGE);
      raise(SIGILL);
      return 0;
    }
  }
  if (shared->vector_item_used + n > kexe_vector_item_budget) {
    arena_exhausted(KEXE_BUDGET_CELLS_VECTOR_ITEMS);
    raise(SIGILL);
    return 0;
  }
  ...
  shared->vector_item_used += n;
  int64_t result = intern_vector(shared, destination, n);
  if (result == 0) { arena_exhausted(KEXE_BUDGET_CELLS_VECTOR_TABLE); raise(SIGILL); return 0; }
  return result;
}
...
static int64_t checked_bytes_slice(struct kexe_context_v11 *context,
  ...
  if (start < 0 || end < start || (uint64_t)end > vector->length) {
    note_value_trap(KEXE_VALUE_TRAP_BYTES_SLICE_BOUNDS);
    raise(SIGILL);
    return 0;
  }
  int64_t result = intern_vector(shared, vector->offset + (uint64_t)start,
                                 (uint64_t)(end - start));
  if (result == 0) { arena_exhausted(KEXE_BUDGET_CELLS_VECTOR_TABLE); raise(SIGILL); return 0; }
  return result;
}
...
static int64_t checked_bytes_concat(struct kexe_context_v11 *context,
  ...
  uint64_t a_offset = a->offset, a_length = a->length;
  uint64_t b_offset = b->offset, b_length = b->length;
  if (a_length + b_length > KEXE_BYTES_VALUE_LIMIT) {
    note_value_trap(KEXE_VALUE_TRAP_BYTES_TOO_LARGE);
    raise(SIGILL);
    return 0;
  }
  if (shared->vector_item_used + a_length + b_length > kexe_vector_item_budget) {
    arena_exhausted(KEXE_BUDGET_CELLS_VECTOR_ITEMS);
    raise(SIGILL);
    return 0;
  }
  uint64_t destination = shared->vector_item_used;
  memmove(&shared->vector_items[destination], &shared->vector_items[a_offset],
          (size_t)a_length * sizeof(int64_t));
  memmove(&shared->vector_items[destination + a_length], &shared->vector_items[b_offset],
          (size_t)b_length * sizeof(int64_t));
  shared->vector_item_used += a_length + b_length;
  int64_t result = intern_vector(shared, destination, a_length + b_length);
  if (result == 0) { arena_exhausted(KEXE_BUDGET_CELLS_VECTOR_TABLE); raise(SIGILL); return 0; }
  return result;
}
...
static int64_t checked_arena_leave(struct kexe_context_v11 *context) {
  ...
  /* Marks never exceed the current use (allocation only grows between
   * enter and leave), so each reset moves backwards or not at all; a mark
   * ahead of the use would mean the struct was written outside this file,
   * and is refused rather than applied. */
  if (mark[0] > shared->pair_used || mark[1] > shared->string_pool_used ||
      mark[2] > shared->vector_used || mark[3] > shared->vector_item_used) {
    raise(SIGILL);
    return 0;
  }
...
static int64_t checked_string_compare(struct kexe_context_v11 *context,
  ...
  int64_t offset_b = checked_pair_get(context, handle_b, 0);
  int64_t length_b = checked_pair_get(context, handle_b, 1);
  if (length_a < 0 || length_b < 0) { raise(SIGILL); return 0; }
  const uint8_t *a = resolve_string_bytes(context, offset_a, length_a);
  const uint8_t *b = resolve_string_bytes(context, offset_b, length_b);
  if (a == NULL || b == NULL) { raise(SIGILL); return 0; }
  if (!ensure_valid_string(context, handle_a, a, length_a) ||
      !ensure_valid_string(context, handle_b, b, length_b)) {
    raise(SIGILL);
    return 0;
  }
  /* Byte order over the common prefix, then the shorter first: memcmp on
   * unsigned bytes IS code point order for canonical UTF-8. */
  size_t common = (size_t)(length_a < length_b ? length_a : length_b);
...
static int64_t checked_string_fold_ascii(struct kexe_context_v11 *context,
  ...
  uint64_t pool_offset = shared->string_pool_used;
  uint8_t *out = shared->string_pool + pool_offset;
  for (int64_t i = 0; i < length; i++) {
    uint8_t c = bytes[i];
    out[i] = (c >= 'A' && c <= 'Z') ? (uint8_t)(c + 32) : c;
  }
  shared->string_pool_used += (uint64_t)length;
  /* A-Z to a-z touches ASCII bytes only, so a valid input folds to a valid
   * output. */
  return mark_validated(context, checked_pair_new(context, -((int64_t)pool_offset) - 1, length));
}

static int kexe_blank_byte(uint8_t c) {
  return c == ' ' || c == '\t' || c == '\n' || c == '\v' || c == '\f' || c == '\r';
}

/* The shared shape of find-blank / skip-blank: FROM must be a code-point
 * boundary in [0, length]; scan for the first byte at or after it whose
 * blankness equals WANTED, answering MISSING when none. Blanks are ASCII,
 * so the scan never has to stop inside a multi-byte sequence to answer,
 * and a continuation byte is never a blank. */
static int64_t checked_string_scan_blank(struct kexe_context_v11 *context,
  ...
  if (!ensure_valid_string(context, handle, bytes, length)) { raise(SIGILL); return 0; }
  if (from < length && (bytes[from] & 0xc0) == 0x80) { raise(SIGILL); return 0; }
  for (int64_t i = from; i < length; i++) {
    if (kexe_blank_byte(bytes[i]) == wanted) return i;
  }
  return missing_is_length ? length : -1;
}

static int64_t checked_string_find_blank(struct kexe_context_v11 *context,
                                         int64_t handle, int64_t from) {
  return checked_string_scan_blank(context, handle, from, 1, 0);
}

static int64_t checked_string_skip_blank(struct kexe_context_v11 *context,
                                         int64_t handle, int64_t from) {
  return checked_string_scan_blank(context, handle, from, 0, 1);
}
...
static int parse_allow(const char *text, uint64_t allow[4]) {
  ...
  while (*cursor) {
    ...
    /* The domain is decide op 9's; `id / 64 >= 4` is ALLOW's array bound. */
    if (errno == ERANGE || end == cursor ||
        kexe_decide(KEXE_DECIDE_WIRE_ID, (int64_t)id, 0, 0, 0) != 1 || id / 64 >= 4 ||
        (*end != ',' && *end != '\0')) return -1;
...
static void trap_handler(int signal_number) {
  /* The OS's signal number -> the generated table's index; the NAME is
   * kexe_loader_names.kotoba's trap-line. */
  unsigned line = KEXE_TRAP_LINE_SIGNAL_UNKNOWN;
  switch (signal_number) {
    case SIGILL: line = KEXE_TRAP_LINE_SIGILL; break;
    case SIGTRAP: line = KEXE_TRAP_LINE_SIGTRAP; break;
    case SIGFPE: line = KEXE_TRAP_LINE_SIGFPE; break;
    case SIGBUS: line = KEXE_TRAP_LINE_SIGBUS; break;
    case SIGSEGV: line = KEXE_TRAP_LINE_SIGSEGV; break;
    case SIGXCPU: line = KEXE_TRAP_LINE_SIGXCPU; break;
    case SIGALRM: line = KEXE_TRAP_LINE_SIGALRM; break;
#if defined(SIGSYS)
    case SIGSYS: line = KEXE_TRAP_LINE_SIGSYS; break;
#endif
    default: break;
  }
...
static void fault_handler(int signal_number, siginfo_t *info, void *context) {
  ...
  if (near_stack_floor(address, 1024u * 1024u) ||
    ...
    write_trap_line(KEXE_TRAP_LINE_HOST_STACK_EXHAUSTED);
    _exit(120);
  }
...
static int supervise(pid_t child) {
  ...
  if (supervisor_timed_out) {
    write_trap_line(KEXE_TRAP_LINE_WALL_TIMEOUT);
    return 122;
  }
  ...
  write_trap_line(KEXE_TRAP_LINE_UNHANDLED_CHILD_SIGNAL);
  return 123;
}
...
static void report_budget_trap(const struct kexe_shared_v11 *shared, int child_status) {
  ...
  switch (shared->budget_exhausted) {
    case KEXE_BUDGET_CELLS_PAIRS: line = KEXE_TRAP_LINE_BUDGET_PAIRS; break;
    case KEXE_BUDGET_CELLS_VECTOR_TABLE: line = KEXE_TRAP_LINE_BUDGET_VECTORS; break;
    case KEXE_BUDGET_CELLS_VECTOR_ITEMS: line = KEXE_TRAP_LINE_BUDGET_VECTOR_ITEMS; break;
  ...
  if (line >= 0) write_trap_line((unsigned)line);
  /* ABI v11: a named VALUE trap, recorded by the refusing host call. */
  if (shared->value_trap > 0 && shared->value_trap < KEXE_VALUE_TRAP_COUNT) {
    const char *name = kexe_value_trap_names[shared->value_trap];
    write_trap_line(KEXE_TRAP_LINE_VALUE_PREFIX);
    write_stderr_checked(name, strlen(name));
    write_stderr_checked("}\n", 2);
  }
}

static int write_supervisor_report(const struct kexe_shared_v11 *shared,
                                   int child_status,
  ...
  if (child_status == 0 && shared->completed == 1) {
    if (strcmp(result_type, kexe_result_type_names[1]) == 0) {
      uint64_t length = 0;
      const uint8_t *bytes = inspect_string_result(shared, shared->result, &length);
      if (bytes == NULL) {
        write_trap_line(KEXE_TRAP_LINE_RESULT_STRING);
      ...
      if (!inspect_record_result(shared, shared->result,
        ...
        write_trap_line(KEXE_TRAP_LINE_RESULT_RECORD);
      ...
      printf("] :fuel {:initial %" PRIu64 " :remaining %" PRIu64
             KEXE_REPORT_TAIL_FMT,
             kexe_initial_fuel, shared->context.fuel, KEXE_REPORT_TAIL_ARGS(shared));
    } else if (strcmp(result_type, kexe_result_type_names[2]) == 0 ||
               strcmp(result_type, kexe_result_type_names[3]) == 0) {
      int option = strcmp(result_type, kexe_result_type_names[2]) == 0;
      int64_t tag, payload;
      if (!inspect_tagged_i64_result(shared, shared->result, option,
                                     &tag, &payload)) {
        int trap_exit = option ? 128 : 129;
        write_trap_line(option ? KEXE_TRAP_LINE_RESULT_OPTION : KEXE_TRAP_LINE_RESULT_RESULT);
      ...
      if (!inspect_variant_result(shared, shared->result, variant_case_count,
        ...
        write_trap_line(KEXE_TRAP_LINE_RESULT_VARIANT);
...
static void probe_denied(unsigned line) {
  write_trap_line(line);
  _exit(124);
}
...
int main(int argc, char **argv) {
  ...
  if (strncmp(result_type, "record:", 7) == 0) {
    /* The rule is decide op 2's; the last clause is the fields[] bound. */
    if (parse_ulong_decimal(result_type + 7, &record_field_count) != 0 ||
        kexe_decide(KEXE_DECIDE_RECORD_FIELDS, (int64_t)record_field_count, 0, 0, 0) != 1 ||
        record_field_count > KEXE_RECORD_FIELD_LIMIT)
      return 2;
  } else if (strncmp(result_type, "variant:", 8) == 0) {
    if (!parse_variant_profile(result_type, &variant_case_count,
                               &variant_bool_mask)) return 2;
  } else if (!kexe_result_type_named(result_type)) return 2;
  int64_t args[6] = {0, 0, 0, 0, 0, 0};
  struct kexe_shared_v11 *shared =
  ...
  if (fuel_env != NULL && fuel_env[0] != '\0' && strcmp(fuel_env, "off") != 0) {
    if (parse_u64(fuel_env, &kexe_initial_fuel) != 0 ||
        kexe_decide(KEXE_DECIDE_BUDGET, KEXE_BUDGET_ID_FUEL, (int64_t)kexe_initial_fuel, 0, 0) == KEXE_BUDGET_VERDICT_ZERO) {
  ...
  if (kexe_decide(KEXE_DECIDE_BUDGET, KEXE_BUDGET_ID_FUEL, (int64_t)kexe_initial_fuel, 0, 0) ==
      KEXE_BUDGET_VERDICT_ABOVE_CEILING) {
  ...
  if (pool_env != NULL && pool_env[0] != '\0') {
    if (parse_u64(pool_env, &kexe_string_pool_budget) != 0 ||
        kexe_decide(KEXE_DECIDE_BUDGET, KEXE_BUDGET_ID_STRING_POOL, (int64_t)kexe_string_pool_budget, 0, 0) == KEXE_BUDGET_VERDICT_ZERO) {
  ...
  if (pairs_env != NULL && pairs_env[0] != '\0') {
    if (parse_u64(pairs_env, &kexe_pair_budget) != 0 ||
        kexe_decide(KEXE_DECIDE_BUDGET, KEXE_BUDGET_ID_PAIRS, (int64_t)kexe_pair_budget, 0, 0) == KEXE_BUDGET_VERDICT_ZERO) {
  ...
  if (vectors_env != NULL && vectors_env[0] != '\0') {
    if (parse_u64(vectors_env, &kexe_vector_budget) != 0 ||
        kexe_decide(KEXE_DECIDE_BUDGET, KEXE_BUDGET_ID_VECTORS, (int64_t)kexe_vector_budget, 0, 0) == KEXE_BUDGET_VERDICT_ZERO) {
  ...
  if (items_env != NULL && items_env[0] != '\0') {
    if (parse_u64(items_env, &kexe_vector_item_budget) != 0 ||
        kexe_decide(KEXE_DECIDE_BUDGET, KEXE_BUDGET_ID_VECTOR_ITEMS, (int64_t)kexe_vector_item_budget, 0, 0) == KEXE_BUDGET_VERDICT_ZERO) {
  ...
  if (cpu_env != NULL && cpu_env[0] != '\0') {
    if (parse_u64(cpu_env, &kexe_cpu_seconds) != 0 ||
        kexe_decide(KEXE_DECIDE_BUDGET, KEXE_BUDGET_ID_CPU_SECONDS, (int64_t)kexe_cpu_seconds, 0, 0) == KEXE_BUDGET_VERDICT_ZERO) {
  ...
  if (wall_env != NULL && wall_env[0] != '\0') {
    if (parse_u64(wall_env, &kexe_wall_seconds) != 0 ||
        kexe_decide(KEXE_DECIDE_BUDGET, KEXE_BUDGET_ID_WALL_SECONDS, (int64_t)kexe_wall_seconds, 0, 0) == KEXE_BUDGET_VERDICT_ZERO) {
      fprintf(stderr, "kexe-loader: KEXE_WALL_SECONDS must be a positive decimal integer\n");
      return 2;
    }
  }
#endif
  if (kexe_decide(KEXE_DECIDE_BUDGET, KEXE_BUDGET_ID_CPU_SECONDS, (int64_t)kexe_cpu_seconds, 0, 0) ==
          KEXE_BUDGET_VERDICT_ABOVE_CEILING ||
      kexe_decide(KEXE_DECIDE_BUDGET, KEXE_BUDGET_ID_WALL_SECONDS, (int64_t)kexe_wall_seconds, 0, 0) ==
          KEXE_BUDGET_VERDICT_ABOVE_CEILING) {
    fprintf(stderr, "kexe-loader: KEXE_CPU_SECONDS / KEXE_WALL_SECONDS exceed the %u-second ceiling\n",
            (unsigned)KEXE_SECONDS_MAX);
    return 2;
  }
  if (kexe_decide(KEXE_DECIDE_BUDGET, KEXE_BUDGET_ID_PAIRS, (int64_t)kexe_pair_budget, 0, 0) ==
      KEXE_BUDGET_VERDICT_ABOVE_CEILING) {
    fprintf(stderr, "kexe-loader: KEXE_PAIRS exceeds the %u-entry ceiling\n",
            (unsigned)KEXE_PAIR_MAX);
    return 2;
  }
  if (kexe_decide(KEXE_DECIDE_BUDGET, KEXE_BUDGET_ID_VECTORS, (int64_t)kexe_vector_budget, 0, 0) ==
      KEXE_BUDGET_VERDICT_ABOVE_CEILING) {
    fprintf(stderr, "kexe-loader: KEXE_VECTORS exceeds the %u-entry ceiling\n",
            (unsigned)KEXE_VECTOR_MAX);
    return 2;
  }
  if (kexe_decide(KEXE_DECIDE_BUDGET, KEXE_BUDGET_ID_VECTOR_ITEMS, (int64_t)kexe_vector_item_budget, 0, 0) ==
      KEXE_BUDGET_VERDICT_ABOVE_CEILING) {
    fprintf(stderr, "kexe-loader: KEXE_VECTOR_ITEMS exceeds the %u-word ceiling\n",
            (unsigned)KEXE_VECTOR_ITEM_MAX);
    return 2;
  }
  if (kexe_decide(KEXE_DECIDE_BUDGET, KEXE_BUDGET_ID_STRING_POOL, (int64_t)kexe_string_pool_budget, 0, 0) ==
      KEXE_BUDGET_VERDICT_ABOVE_CEILING) {
    fprintf(stderr, "kexe-loader: KEXE_STRING_POOL exceeds the %u-byte ceiling\n",
            (unsigned)KEXE_STRING_POOL_MAX);
    return 2;
  }
  /* The shared mapping's arrays are sized by these maxima. Every budget
   * above was ruled by decide op 7; this is the arrays' bound, which holds
   * whenever that verdict does, so it answers nothing a run can reach. */
  if (kexe_pair_budget > KEXE_PAIR_MAX || kexe_vector_budget > KEXE_VECTOR_MAX ||
      kexe_vector_item_budget > KEXE_VECTOR_ITEM_MAX ||
      kexe_string_pool_budget > KEXE_STRING_POOL_MAX) {
    fprintf(stderr, "kexe-loader: a budget exceeds the shared mapping\n");
    return 2;
  }
  ...
  shared->context.arena_enter = checked_arena_enter;
  shared->context.arena_leave = checked_arena_leave;
  shared->context.string_compare = checked_string_compare;
  shared->context.string_fold_ascii = checked_string_fold_ascii;
  shared->context.string_find_blank = checked_string_find_blank;
  shared->context.string_skip_blank = checked_string_skip_blank;
  shared->context.string_index_of_from = checked_string_index_of_from;
  shared->context.string_compare_lines = checked_string_compare_lines;
  shared->context.string_find_byte = checked_string_find_byte;
  shared->context.string_append_range = checked_string_append_range;
  /* ABI v11: the string constructor and the three bytes slots. */
  shared->context.string_from_utf8 = checked_string_from_utf8;
  shared->context.bytes_from_vector = checked_bytes_from_vector;
  shared->context.bytes_slice = checked_bytes_slice;
  shared->context.bytes_concat = checked_bytes_concat;
  /* ABI v9: the six pointers the inline handle operations read. */
  ...
  shared->context.vectors_base = shared->vectors;
  ...
  if (kexe_cap_admitted(shared->context.allow, 42)) {
  ...
  if (getenv("KEXE_FILESYSTEM_PROBE") != NULL) {
    ...
    probe_denied(KEXE_TRAP_LINE_SANDBOX_FILESYSTEM);
  }
  if (getenv("KEXE_NETWORK_PROBE") != NULL) {
    int probe = socket(AF_INET, SOCK_STREAM, 0);
    if (probe < 0) probe_denied(KEXE_TRAP_LINE_SANDBOX_NETWORK);
    ...
    (void)close(probe);
    probe_denied(KEXE_TRAP_LINE_SANDBOX_NETWORK);
  }
  if (getenv("KEXE_PROCESS_PROBE") != NULL) {
    ...
    probe_denied(KEXE_TRAP_LINE_SANDBOX_PROCESS);
  }
  int64_t result;
