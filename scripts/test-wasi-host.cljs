...
(let [tmp (.mkdtempSync fs (.join path (.tmpdir os) "kotoba-wasi-host-"))
      ...
      compile! (fn [source output & args]
                 (let [result (.spawnSync child js/process.execPath
                   ...
                   (lib/ensure! (zero? (or (.-status result) 70))
                                (str "WASI compilation failed: " (.-stderr result)))))]
  (try
    (compile! (.join path lib/root "examples" "structured.kotoba") (.join path tmp "program.wasm"))
    ...
    (let [result (.spawnSync child js/process.execPath
                             #js [(.join path lib/root "scripts" "test-wasi-host.mjs")
                                  (.join path tmp "program.wasm")
                                  (.join path tmp "capability.wasm")]
                             #js {:cwd lib/root :encoding "utf8" :maxBuffer 1048576})]
      ...
      (when (.-error result) (throw (.-error result)))
      (lib/ensure! (zero? (or (.-status result) 70)) "WASI host ES module test failed"))
