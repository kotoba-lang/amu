...
try {
  ...
  send({ id: "wrong-profile",
    args: ["compile", "examples/i64-semantics.kotoba", "--target", "wasm32-wasi",
      "--output", join(directory, "wrong-profile.wasm")] });
  const wrongProfile = await next();
  if (wrongProfile.status !== 64 || !wrongProfile.stderr.includes("locked to target")) {
    throw new Error("worker accepted a different profile from the locked target");
  }
