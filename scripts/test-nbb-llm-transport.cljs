(ns scripts.test-nbb-llm-transport
  "Launcher for `test/nbb/llm-transport.cljs` (ADR 0118), mirroring
  `scripts/test-nbb-http-transport.cljs`."
...
(let [resolved (lib/run "clojure" ["-Spath" "-M:test"])
      ...
      result (.spawnSync child js/process.execPath
                         (clj->js [nbb-cli "--classpath" classpath
                                   (lib/join lib/root "test" "nbb" "llm-transport.cljs")])
