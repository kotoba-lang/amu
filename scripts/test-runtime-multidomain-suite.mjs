...
function bridgeReport(report, expectSuccess = true) {
  ...
  const result = spawnSync("nbb", [join(root, "scripts", "perfgate-qualify.cljk"), input],
...
function validateManifestV2(value, expectSuccess = true) {
  ...
  const result = spawnSync("nbb", [join(root, "scripts", "perfgate-qualify.cljk"),
...
try {
  ...
  if (report.contract.workflow !== "prepare-all -> bounded quiet gate -> measure sealed bundles"
      ...
      || report.qualification.quietGate.limit !== 0.10
