...
## 分担 (kotoba-cloud-maint — kotoba.cloud メンテ)

担当 repo: ~/github/com-junkawasaki/orgs/kotoba-lang/app-kotoba-cloud
(= kotoba.cloud control-plane, Cloudflare Worker: build/worker.js + assets)。

1 回の実行 (60min cron) の仕事:
1. `gh api repos/network-awai/app-kotoba-cloud/actions/runs` (存在すれば) と
   live site の健全性を確認: curl https://kotoba.cloud の status,
   /404-page の挙動, worker の deploy 新鮮度。
   site が落ちている/直前 deploy が壊れている場合は最小情報を集めて報告
   (deploy の rollback は人間承認が必要なので自分では実行しない)。
2. repo の triage: open issue / open PR / README と実装の乖離。
   deps (wrangler, chicory 等) の更新 PR が必要なら提案 issue を立てる。
3. 修正 PR は branch bot/maint-<日時> から。main 直 push・deploy 実行は禁止
   (control-plane なので事故の影響が大きい。提案と PR まで)。
4. 報告: site 健全性 / triage 件数 / 出した PR・issue。誇張なし。
